...
public class ZombieBase : MonoBehaviour
{
    ...
    public void ActionDecision()
    {
        ...
        if (nearthDistrubtor != null)
        {
            ...
            MoveToAttackAsync(nearthDistrubtor.currentTile).Forget();
            return;
        }

        if (isChasingPlayer && !MapController.Instance.Player.GetIsClocking())
        {
            //Debug.Log(gameObject.name + "가 플레이어를 발견했습니다!");
            MoveToAttackAsync(MapController.Instance.Player.TileController.Model).Forget();

            // 플레이어 바라보기
            var updatePos = MapController.Instance.Player.transform.position;
            updatePos.y += 0.6f;
        ...
        else
        {
            ...
            if (randomInt == 0)
            {
                MoveToRandomAsync().Forget();
            }
        }
    }

    public async UniTask MoveToAttackAsync(Tile target, float time = 0.25f)
    {
        ...
        if (movePath.Count == 0 && target == MapController.Instance.Player.TileController.Model)
        {
            // 플레이어가 1칸 내에 있는 경우
            AttackPlayer(MapController.Instance.Player);
        }
        else
        {
            if (movePath.Count == 0)
    ...
    public async UniTask MoveToRandomAsync(int num = 1, float time = 0.25f)
    {
        ...
        if (candidate[rand] == MapController.Instance.Player.TileController.Model)
            rand--;
